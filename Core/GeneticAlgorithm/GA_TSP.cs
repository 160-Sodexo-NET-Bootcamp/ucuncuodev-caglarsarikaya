...
namespace Core.GeneticAlgorithm
{
    public class GA_TSP
    {
        ...
        [STAThread]
        public void Run(List<ContainerCluster>  list)
        {
            GA_TSP tsp = new GA_TSP();

            tsp.Initialization(list);

            tsp.TSPCompute();
        }

        public void Initialization(List<ContainerCluster> list)
        {
            Random randObj = new Random();
            try
            {
                cityCount = list.Count();
                populationSize = 1000;
            ...
            {
                cities[i] =
                    new City((int)list[i].LocationX,(int)list[i].LocationY);
            }
            ...
            while (countSame < 120)
            {
                ...
                for (int i = 0; i < favoredPopulationSize; i++)
                {
                    ...
                    int father =
                        (
                        int
                        )(randObj.NextDouble() * (double)matingPopulationSize);
                ...
                double mutationRate =
                    100.0 * (double)mutated / (double)matingPopulationSize;
